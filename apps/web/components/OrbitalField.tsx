...
/*                                                                            */
/*  --- On living in this repo ---------------------------------------------- */
/*                                                                            */
/*  This arrived as a shadcn-shaped drop-in for /components/ui. This app is    */
/*  not a shadcn project and has no such folder, so it sits with the rest of   */
/*  the components under the repo's own PascalCase convention.                 */
/*                                                                            */
/*  The bodies were re-coloured to the app's palette. The stock set is a full  */
/*  spectrum -- blue Earth, red Mars, orange Jupiter -- which would have been  */
/*  the only place in the product where colour is decorative rather than       */
/*  semantic. Here the coils are engraved gold, and exactly one body carries   */
/*  the contraction red, so the hero reads in the same two colours as every    */
/*  regime badge, chart stroke and verdict pill.                              */
/* -------------------------------------------------------------------------- */
...
export interface OrbitalFieldProps extends React.HTMLAttributes<HTMLDivElement> {
  /** Bodies to run. Defaults to the plate set below. */
  planets?: Planet[];
...
/*  The plate set                                                             */
/*                                                                            */
/*  Real J2000 elements — the geometry is untouched. Only the colours are      */
/*  ours: five weights of the engraving gold, and the contraction red on a     */
/*  single body so there is one cold line among the warm ones.                 */
/*                                                                            */
/*  The two semantic colours come from lib/palette.ts rather than being        */
/*  written out here, for the reason that file records: the red was lifted     */
/*  once for contrast and three hand-copied duplicates were missed.            */
/* -------------------------------------------------------------------------- */

/** Foil highlight and its shadow. Decorative shades, not semantic colours. */
const FOIL = "#E8C964";
const FOIL_DEEP = "#B8912F";
const PAPER = "#F4F2EC";

export const PLATE_BODIES: Planet[] = [
  { name: "Mercury", a: 0.38710, e: 0.20563, i: 7.005, node: 48.331, peri: 29.125, M0: 174.796, color: PAPER, size: 2.2 },
  { name: "Venus",   a: 0.72333, e: 0.00677, i: 3.395, node: 76.680, peri: 54.853, M0: 50.115,  color: FOIL, size: 3.2 },
  { name: "Earth",   a: 1.00000, e: 0.01671, i: 0.000, node: 348.739, peri: 114.208, M0: 357.517, color: EXPANSION, size: 3.8, glow: 1.15 },
  { name: "Mars",    a: 1.52371, e: 0.09339, i: 1.850, node: 49.558, peri: 286.483, M0: 19.373, color: CONTRACTION, size: 2.9 },
  { name: "Jupiter", a: 5.20290, e: 0.04839, i: 1.303, node: 100.464, peri: 273.867, M0: 20.020, color: FOIL, size: 5.0, glow: 0.9 },
  { name: "Saturn",  a: 9.53700, e: 0.05386, i: 2.485, node: 113.665, peri: 339.392, M0: 317.020, color: FOIL_DEEP, size: 4.6, glow: 0.85 },
];
...
export function OrbitalField({
  ...
  scrim = "none",
  scrimStrength = 0.88,
  // Lower than the original's 1500. The plate is a backdrop for type here,
  // and a dense star field competes with it; this also keeps the per-frame
  // star loop cheap on the phones the responsive gate runs at.
  starCount = 900,
  glow = 1,
  showOrbits = false,
  showSunTrack = true,
  ...
  return (
    <div
      ref={hostRef}
      className={`relative isolate h-full w-full overflow-hidden bg-ink ${className}`}
      {...rest}
    >
